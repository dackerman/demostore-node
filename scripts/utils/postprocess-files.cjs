...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? '@dackerman-stainless/demostore/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@dackerman-stainless/demostore/_shims/index'`,
